...
def get_admin_organizations():
    ...
    try:
        ...
        logger.error(
            f"Ошибка получения организаций: {type(e).__name__}: {e}", exc_info=True
        )
        return jsonify({"error": "Внутренняя ошибка сервера"}), 500
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            try:
                ...
                return jsonify(
                    {"message": "Категория успешно создана", "category": category}
                ), 201
...
def owner_register():
    ...
    try:
        ...
        if not full_name or not email or not password or not organization_name:
            return jsonify(
                {"error": "Имя, email, пароль и название организации обязательны"}
            ), 400
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            ...
            """,
                (
                    "update_org",
                    psycopg2.extras.Json(pending_data),
                    request.current_owner["email"],
                ),
            )
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            ...
            if req["request_type"] == "new_org":
                ...
                cursor.execute(
                    ...
                    RETURNING id
                """,
                    (data["organization_name"], "", "", "", embedding),
                )
...
def create_test_categories():
    ...
    categories = [
        ...
        (
            "Работа и карьера",
            "Центры занятости, кадровые агентства, обучение профессиям",
        ),
...
def create_default_admin():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            conn.commit()
            logger.info(
                "✅ Администратор по умолчанию успешно создан: username='admin', email='admin@tyumen-resources.ru'"
            )
    except Exception as e:
